...
use Simqel\Connection_MySQL;
use Simqel\Settings;

/**
 * Class SqlConnectionMySQLTest
...
class SqlConnectionMySQLTest extends TestCase
{
    ...
    protected function setUp()
    {
        if (!extension_loaded('mysql')) {
            $this->markTestSkipped('mysql module is not present.');
        }
        $db = MYSQL_DB;
    ...
    {
        $settings = $this->getMock('Simqel\Settings', array(), array('dsn'), 'MockSettings', false);
        $connection = new Connection_MySQL($settings);
        $handle = 'some handle';
